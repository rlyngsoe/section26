...
ARG *eventlist2history(AnnotatedGenes *a, FILE *output)
{
  ...
  if ((eventlist != NULL) && (Length(eventlist) > 0)){
	 ...
	 */
	fprintf(stderr, "Error in backtracking - please email error report\n");
	exit(1);
	break;
      case COLLAPSE:
	...
	if (output != NULL){
	  ...
	  if (t != NULL)
	    fprintf(output,
		    "Recombination in sequence %s after site %s; %sfix is new sequence %d\n",
		    t, s, pfix, next_seq);
	  else
	    fprintf(output,
		    "Recombination in sequence %d after site %s; %sfix is new sequence %d\n",
		    j + 1, s, pfix, next_seq);
	}
	/* Clean up */
	if (a->positions == NULL)
...
{
  LListCounter *lcounter = MakeCounter(h->event, FIRST);
  Event *e;

  while ((e = (Event *)Next(lcounter)) != NULL){
    switch (e->type){
    case SUBSTITUTION:
      printf("Substitution seq: %d, site: %d\n", e->event.s.seq, e->event.s.site);
      break;
    case COALESCENCE:
      printf("Coalescence %d %d\n", e->event.c.s1, e->event.c.s2);
      break;
    case RECOMBINATION:
      printf("Recombination seq: %d pos: %d\n", e->event.r.seq, e->event.r.pos);
      break;
    case REMOVE:
      printf("Remove %d\n", e->event.remove);
      break;
