/*******************************************************************
    ...
    Revision History:
      September 2006: Added marginal trees as output format
			      ...
			      FILE *fp)
{
  ...
  for (i = 0; i < Length(llist); i++){
    ...
    if (!maiden)
      fprintf(fp, ";");
    if (a->positions != NULL)
      fprintf(fp, (char *)GetByIndex(a->positions, (int)p));
    else
		...
		ARGLabels nodelabels, int annotate_edges, int generate_id, ...)
{
  ...
  for (i = 0; i < arg->n; i++){
	...
	fprintf(fp, arg->nodes[i].sequence);
      maiden = 0;
    }
    if (maiden){
